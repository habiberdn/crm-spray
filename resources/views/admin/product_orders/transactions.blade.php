                ...
                @forelse($my_transactions as $transaction)
                    ...
                    <div class="item-product flex flex-row justify-between items-center">
                        ...
                        <div>
                            ...
                            @if ($transaction->id == $diskons[$loop->index]->product_id)
                                @if ($diskons[$loop->index]->type == 'percentage')
                                    <p class="text-indigo-950 font-bold text-xl">Rp
                                        {{ number_format($transaction->product->price - ($transaction->product->price * (int) $diskons[$loop->index]->value) / 100) }}
                                    </p>
                                @elseif($diskons[$loop->index]->type == 'percentage')
                                    <p class="text-indigo-950 font-bold text-xl">Rp
                                        {{ number_format($transaction->product->price - (int) $diskons[$loop->index]->value) }}
                                    </p>
                                @endif
                            @else
                                <p class="text-indigo-950 font-bold text-xl">Rp
