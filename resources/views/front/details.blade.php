...
@section('content')
    ...
    <header class="w-full pt-[74px] pb-[103px] relative z-0">
        <div class="container max-w-[1130px] mx-auto flex flex-col z-10 px-4">
            <div class="flex flex-col gap-4 mt-7 z-10">
                <p class="bg-pink-600 border border-pink-600 rounded-xl font-semibold text-white px-4 py-2 w-fit text-sm sm:text-base leading-tight">
    ...
    <section id="DetailsContent" class="container max-w-[1130px] mx-auto mb-8 relative -top-[70px] px-4">
        <div class="flex flex-col gap-8">
            <div class="w-full h-[250px] sm:h-[400px] md:h-[500px] lg:h-[700px] flex shrink-0 rounded-[20px] overflow-hidden shadow-2xl">
            ...
            <div class="flex flex-col lg:flex-row gap-8 relative -mt-[40px] lg:-mt-[93px]">
                <!-- Overview -->
                <div class="flex flex-col p-6 gap-5 bg-gradient-to-br from-pink-600 to-pink-700 rounded-[20px] w-full lg:w-[700px] shrink-0 mt-6 lg:mt-[90px] h-fit shadow-xl">
                ...
                <div class="flex flex-col w-full lg:w-[366px] gap-6 lg:gap-[30px] flex-nowrap overflow-y-visible">
                            ...
                            <div class="w-full p-6 bg-white rounded-[20px] flex flex-col gap-6">
                                <div class="flex flex-col gap-3">
                                    ...
                                    @if ($product->diskon->type == 'percentage')
                                        <span class="backdrop-blur bg-pink-500 rounded-[4px] px-2 py-1 text-lg sm:text-xl font-bold text-white shadow-lg w-fit">
                                            Rp {{ number_format($product->price - ($product->price * $product->diskon->value) / 100) }}
                                        </span>
                                ...
                                <p class="text-sm text-gray-600">
                                    ...
                                    <a href="https://wa.link/gfhop2" class="text-green-500 hover:text-green-600 font-semibold">Whatsapp</a>
                                </p>
                            </div>
                        </div>
                    @else
                        <div class="p-[2px] bg-gradient-to-br from-pink-400 to-pink-500 rounded-[20px] flex w-full h-fit shadow-xl">
                            <div class="w-full p-6 bg-white rounded-[20px] flex flex-col gap-6">
                                <div class="flex flex-col gap-3">
                                    <p class="font-semibold text-2xl sm:text-3xl lg:text-4xl bg-clip-text bg-gradient-to-r from-pink-500 to-pink-600 text-transparent">
                                        Rp {{ number_format($product->price) }}
                                    </p>
                    ...
                    <!-- Creator -->
                    <div class="w-full p-6 bg-pink-50 border border-pink-200 rounded-[20px] flex flex-col gap-4 h-fit shadow-lg">
                            ...
                            <div class="flex gap-3 items-center">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <p class="text-gray-600 text-sm leading-5">
                                        <span class="font-semibold mr-1 text-pink-600">{{ count($creator_products) }}</span>
                                        Product
                                    </p>
                                </div>
                            </div>
    ...
    <section id="NewProduct" class="container max-w-[1130px] mx-auto mb-[102px] flex flex-col gap-8 px-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            @forelse($other_products as $product)
                ...
                <div class="product-card flex flex-col rounded-[18px] bg-pink-50 overflow-hidden hover:transform hover:scale-105 transition-all duration-300 border border-pink-100 shadow-lg">
                    <!-- Product Thumbnail -->
                    <a href="{{ route('front.details', $product->slug) }}"
                        ...
                        <div class="absolute top-2 sm:top-3 right-2 sm:right-[14px] z-10 flex flex-col items-end gap-1">
                            @if ($hasDiscount)
                                ...
                                <span class="backdrop-blur bg-pink-500 rounded-[4px] px-2 py-1 text-xs sm:text-sm font-bold text-white shadow-lg">
                                    Rp {{ number_format($finalPrice) }}
                                </span>
                            @else
                                <p class="backdrop-blur bg-pink-100/80 rounded-[4px] p-[4px_8px] text-xs sm:text-sm font-medium text-gray-800">
                                    Rp {{ number_format($product->price) }}
                                </p>
                            @endif
                        </div>
                    ...
                    <div class="p-3 sm:p-[10px_14px_12px] h-full flex flex-col justify-between gap-3 sm:gap-[14px]">
                        <div class="flex flex-col gap-1 sm:gap-2">
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                @if ($hasDiscount)
                                    @if ($product->diskon->type == 'percentage')
                                        <p class="bg-pink-500/20 text-pink-600 font-semibold text-xs rounded-[4px] p-[4px_6px] w-fit border border-pink-300">
                        ...
                        <div class="flex items-center gap-2 sm:gap-[6px]">
                            <div class="w-5 h-5 sm:w-6 sm:h-6 flex shrink-0 items-center justify-center rounded-full overflow-hidden border border-pink-200">
                                <img src="{{ Storage::url($product->creator->avatar) }}" class="w-full h-full object-cover"
                                    alt="{{ $product->creator->name }} avatar">
                            </div>
                            <a href="#" class="font-semibold text-xs text-gray-600 hover:text-pink-600 transition-colors duration-200 truncate">
...
@push('after-script')
    <script>
        ...
        @if(session('error'))
            const toast = document.createElement('div');
            toast.className = 'fixed top-20 right-4 bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 animate-slide-in';
